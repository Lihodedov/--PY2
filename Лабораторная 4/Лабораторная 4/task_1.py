class AcElectricMachine:
    ...
    def __init__(self, name: str, voltage: int, current: int, rotation: int, frequency: int):
        ...
        self._voltage_value: int = 0
        self._current_value: int = 0
        self._rotation_value: int = 0
        self._frequency_value: int = 0
    ...
    # Защищаем текущие значения параметров электрических машин переменного тока
    @property
    def voltage_value(self) -> int:
        """Возвращает текущее значение напряжения."""
        return self._voltage_value

    @property
    def current_value(self) -> int:
        """Возвращает текущее значение тока."""
        return self._current_value

    @property
    def rotation_value(self) -> int:
        """Возвращает текущее значение скорости вращения."""
        return self._rotation_value

    @property
    def frequency_value(self) -> (int, float):
        """Возвращает текущее значение частоты тока."""
        return self._frequency_value

    # Защищаем номинальные параметры электрических машин переменного тока
    @property
    def name(self) -> str:
    ...
    def switch_on(self) -> None:
        """Метод включает электрическую машину, возвращает f-строку."""
        # Исходные текущие значения возрастают до номинальных
        self._voltage_value += self._voltage
        self._current_value += self._current
        self._rotation_value += self._rotation
        self._frequency_value += self._frequency

    def switch_off(self) -> None:
        """Метод отключает электрическую машину, возвращает f-строку."""
        # Текущие значения параметров снижаются до нуля.
        self._voltage_value = 0
        self._current_value = 0
        self._rotation_value = 0
        self._frequency_value = 0

    def display_voltage(self) -> None:
        """Метод показывает текущее значение напряжения электрической машины переменного тока."""
        if not isinstance(self._voltage_value, int):
            raise TypeError("Напряжение должно быть типа int")
        if self._voltage_value < 0:
            raise ValueError("Напряжение не может быть отрицательным числом")
        if self._voltage_value == 0:
            print(f"{self._voltage_value} - электрическая машина отключена")
        if self._voltage_value < (0.9 * self._voltage):
            print(f"{self._voltage_value} - напряжение электрической машины < 0.9 Uном")
        if self._voltage_value > (1.1 * self._voltage):
            print(f"{self._voltage_value} - напряжение электрической машины > 1.1 Uном")
        else:
            print(f"{self._voltage_value} - допустимое значение напряжения электрической машины")

    def display_current(self) -> None:
        """Метод показывает текущее значение тока электрической машины переменного тока."""
        if not isinstance(self._current_value, int):
            raise TypeError("Ток должен быть типа int")
        if self._current_value < 0:
            raise ValueError("Ток не может быть отрицательным числом")
        if self._current_value == 0:
            print(f"{self._current_value} - электрическая машина отключена")
        if self._current_value < self._current:
            print(f"{self._current_value} - электрическая машина работает на холостом ходу")
        if self._current_value > self._current:
            print(f"{self._current_value} - электрическая машина перегружена")
        if self._current_value == self._current:
            print(f"{self._current_value} - ток электрической машины номинальный")
...
class AsynchronousMotor(AcElectricMachine):
    ...
    def __init__(self, name: str, voltage: int, current: int, rotation: int, frequency: int, use_power: int):
        ...
        """
        self._use_power_value = 0  # текущее значение потребляемой мощности (электродвигатель отключен)
    ...
    # Защищаем текущие значения параметров асинхронных электродвигателей
    @property
    def use_power_value(self) -> int:
        """ Возвращает текущее значение потребляемой мощности электродвигателя."""
        return self._use_power_value

    # Защищаем номинальные параметры асинхронных электродвигателей
    @property
    ...
    def __str__(self) -> str:
        """Возвращает f-строку."""
        return super().__str__() + f" Потребляемая мощность {self.use_power}."
    ...
    def switch_on(self) -> None:
        """Метод включает электродвигатель."""
        super().switch_on()
        self._use_power_value += self._use_power  # вносим параметр электродвигателей

    def switch_off(self) -> None:
        """Метод отключает электродвигатель."""
        super().switch_off()
        self._use_power_value = 0

    def display_use_power(self) -> None:
        """Метод показывает текущее значение потребляемой мощности электродвигателя."""
        if not isinstance(self._use_power_value, int):
            raise TypeError("Потребляемая мощность должна быть типа int")
        if self._use_power_value < 0:
            raise ValueError("Потребляемая мощность не может быть отрицательным числом")
        if self._use_power_value == 0:
            print(f"{self._use_power_value} - электродвигатель отключен")
        if self._use_power_value < self._use_power:
            print(f"{self._use_power_value} - электродвигатель работает на холостом ходу")
        if self._use_power_value == self._use_power:
            print(f"{self._use_power_value} - потребляемая мощность номинальная")
        if self._use_power_value > self._use_power:
            print(f"{self._use_power_value} - электродвигатель перегружен")
...
class SynchronousGenerator(AcElectricMachine):
    ...
    def __init__(self, name: str, voltage: int, current: int, rotation: int, frequency: int, gen_power: int):
        ...
        """
        self._gen_power_value = 0  # текущее значение вырабатываемой мощности (генератор отключен)
    ...
    # Защищаем текущие значения параметров синхронных генераторов
    @property
    def gen_power_value(self) -> int:
        """Возвращает текущее значение вырабатываемой мощности генератора."""
        return self._gen_power_value

    # Защищаем номинальные параметры синхронных генераторов
    @property
    ...
    def __str__(self) -> str:
        """Возвращает f-строку."""
        return super().__str__() + f" Вырабатываемая мощность {self.gen_power}."
    ...
    def switch_on(self) -> None:
        """Метод включает генератор."""
        super().switch_on()
        self._gen_power_value += self._gen_power  # вносим параметр генераторов

    def switch_off(self) -> None:
        """Метод отключает генератор."""
        super().switch_off()
        self._gen_power_value = 0
...
if __name__ == "__main__":
    # инициализация экземпляра класса асинхронных электродвигателей
    motor = AsynchronousMotor("AMA 450L4L", 10000, 51, 1489, 50, 710000)

    # выводим первоначальные значения электродвигателя
    print(motor.use_power_value)
    print(motor.voltage_value)

    motor.switch_on()  # запускаем электродвигатель

    # сравниваем с первоначальными значениями (проверяем работу унаследованного, расширенного метода)
    print(motor.use_power_value)
    print(motor.voltage_value)

    motor.display_current()  # выводим текущее значение тока (проверяем работу унаследованного метода)
