...
class Video:
    ...
    def cropping(self, cropping_video: Union[int, float]) -> (int, float):
        """
        Метод обрезает видео.

        :param cropping_video: Обрезаемый фрагмент видео
        ...
        23.11
        """
        if not isinstance(cropping_video, (int, float)):
        ...
        if cropping_video >= self.length:
            raise ValueError("Обрезанный фрагмент должен быть меньше продолжительности видео")
        new_cropping_video = self.length - cropping_video
        return new_cropping_video
